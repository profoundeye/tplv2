    ...
    function init(param, obj) {
        if (param && param instanceof Object) {
            ...
            function getCurrPage() {
                if (options.info.cookie_currPageKey && options.info.cookie_currPage) {
            ...
            function getFirst() {
                ...
                if (options.info.first_on == false) {
                    return ""
                }
            ...
            function getPrev() {
                if (options.info.prev_on == false) {
                    return ""
                }
                return options.info.prev
            }
            function getNext() {
                if (options.info.next_on == false) {
                    return ""
                }
...
function checkBrowser(){
	...
	var msg = '建议安装 firefox、chrome、ie8+、opera、safari 等浏览器，如果您的浏览器有极速功能请开启。';
    var brver=(UA.match(/.+(?:rv|it|ra|ie)[\/: ]([\d.]+)/) || [0, '0'])[1];
    if(UA.indexOf('metasr')>-1){
		msg += '您正在使用搜狗浏览器，请开启急速模式，';
       $('#browser_tip').html(msg).show('fast');
       return;
    }
    if(UA.indexOf('qihu 360ee')>-1 || UA.indexOf('360se')>-1){
	   msg += '您正在使用360浏览器，请使用360告诉浏览器，';
	   $('#browser_tip').html(msg).show('fast');
       return;
...
function load_swf(type,bid,i,title,url){
    shoot_close();
        ...
        var player = '<div class="video_windows"><embed src="'+url+'" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" wmode="opaque" width="480" height="550" ></embed></div>';
        if(obj.is(":hidden")){
...
function shoot_swf(type,idname,title,url){
    ...
    if(type == 'video'){
        swf = '<embed src="'+url+'" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" wmode="opaque" width="510" height="480" ></embed>';
    }
    if(type == 'xiami'){
...
function tmpl_hit(d){
    ...
    a   +=  '<div class="reply_content"><a href="' +d.h_url+ '" target="_blank">' +u+ '</a><li>' +d.title+ '</li></div>';
